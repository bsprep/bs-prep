  ...
  return (
    <div className="flex-1 p-6 w-full max-w-5xl mx-auto flex flex-col min-h-[90vh]">
      ...
      <div className="mb-8 flex justify-between items-center">
        <button 
          onClick={() => router.back()}
          className="flex items-center gap-2 text-sm font-semibold uppercase tracking-widest text-slate-500 hover:text-white transition-colors"
        >
      ...
      {/* Original Doubt */}
      <div className="bg-[#070c15] rounded-2xl border border-white/10 p-6 md:p-10 shadow-sm mb-8">
        ...
        <div className="flex items-center gap-3 mb-8 pb-8 border-b border-white/10">
          ...
          <div>
            <div className="text-sm font-semibold text-slate-100">{doubt.author.full_name}</div>
        ...
        <div className="prose prose-sm max-w-none text-slate-300 font-medium leading-relaxed mb-8 whitespace-pre-wrap">
          {doubt.description}
        </div>
      ...
      <div className="space-y-6 mb-10">
        {replies.map((reply) => (
          <div key={reply.id} className={`bg-[#070c15] rounded-2xl border p-6 shadow-sm ${reply.is_accepted_answer ? 'border-emerald-500/50 bg-emerald-500/10' : 'border-white/10'}`}>
                ...
                <div>
                  <div className="text-sm font-semibold text-slate-100 flex items-center gap-2">
                    ...
                    {reply.is_official_answer && (
                      <span className="flex items-center gap-1 text-[9px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full uppercase tracking-widest">
            ...
            <div className="text-sm text-slate-300 font-medium leading-relaxed whitespace-pre-wrap ml-11">
              {reply.content}
            </div>
          </div>
        ))}
      ...
      {/* Reply Input */}
      {doubt.status !== 'resolved' ? (
        <form onSubmit={handlePostReply} className="bg-[#070c15] rounded-2xl border border-white/10 p-4 shadow-sm flex flex-col">
          <textarea 
            value={newReply}
            onChange={e => setNewReply(e.target.value)}
            placeholder="Type your official reply here..."
            className="w-full h-24 p-4 bg-transparent border-none text-sm font-medium text-slate-100 placeholder:text-slate-600 outline-none resize-none"
            required
          />
          <div className="flex justify-between items-center border-t border-white/5 pt-4">
            <div className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest px-4">
              Posting as Mentor/Admin.
            </div>
            <Button 
              type="submit" 
              disabled={isSubmitting}
              className="bg-white hover:bg-slate-200 text-black rounded-xl h-10 px-6 text-xs font-semibold uppercase tracking-widest shadow-md transition-all"
            >
        ...
        <div className="text-center py-6 bg-white/5 rounded-2xl text-xs font-semibold uppercase tracking-widest text-slate-500">
